...
function normalize(item, category, feedTitle) {
  const img = item?.enclosure?.url || item?.contentSnippet?.match(/(https?:[^\s]+\.(jpg|png|jpeg))/i)?.[1] || null;
...
export async function fetchCategory(category) {
  ...
  for (const url of urls) {
    try {
      ...
      for (const item of feed.items.slice(0, 20)) {
        results.push(normalize(item, category, feed.title || new URL(url).hostname));
      }
    } catch (e) {
      // ignore feed errors
    }
  }
  // Görseli olmayanların küçük bir kısmına sayfadan og:image bul
  const lacking = results.filter(r => !r.image).slice(0, 10);
  await Promise.all(lacking.map(async r => {
  ...
  // tarihe göre sırala
  unique.sort((a,b) => dayjs(b.isoDate).valueOf() - dayjs(a.isoDate).valueOf());
